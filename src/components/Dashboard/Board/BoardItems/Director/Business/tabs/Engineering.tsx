...
const Engineering: FC = () => {
   ...
   const { data, isLoading: isWorksLoading, refetch } = useGetAllBusinessWorkByTypeQuery('engineering')
   ...
   const scroll_row_length = useMemo(() => {
      return COLUMNS_D.SCROLL_VALUES.engineering
         .map((v) => {
            if (typeof v !== 'object') {
               return v
            }
   ...
   // return (
   //    <div className={css.wrapper}>
   //       <Navigation type='engineering' />
   //       <AddWorkButton type='engineering' />
   ...
   //             {COLUMNS_D.SCROLL_VALUES.engineering.map((c, i) => {
